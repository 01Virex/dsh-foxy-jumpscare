 ...
 *   1. localStorage "dsh-foxy-jumpscare.config" (full JSON, same shape as the file)
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      let hideTimer = null;
      let active = false;
      let localOverride = null; // localStorage 覆盖(最高优先级,后于 config.json 应用)
      ...
      const loadRemoteConfig = () => {
        fetch("/plugins/dsh-foxy-jumpscare/config.json", { cache: "no-store" })
          ...
          .then((data) => {
            ...
            // 优先级:defaults <- config.json <- localStorage(最高)
            let next = config;
            if (norm) next = mergeConfig(next, norm);
            if (localOverride) next = mergeConfig(next, localOverride);
            applyConfig(next);
      ...
      const start = () => {
        // localStorage 覆盖(最高优先级;先记下,待 config.json 到达后再叠加回来)
        try {
          const raw = localStorage.getItem(CONFIG_KEY);
          if (raw !== null) {
            const norm = normalizeConfig(JSON.parse(raw));
            if (norm) {
              localOverride = norm;
              config = mergeConfig(config, norm);
            }
          }
        } catch (e) { /* 忽略损坏数据 */ }
